...
def st_attention(x, is_encoder=True, pre_x=None, channels=3, input_len=12):
    ...
    x = tf.concat(
        list(map(channel_combine, [x[:, i : i + channels] for i in range(input_len)])),
        axis=1,
    )
    return x
...
def STHolistic_tf(x, is_encoder=True, pre_x=None, channels=3, input_len=12):
    ...
    x = tf.concat(
        [
            siteCombine_tf(x[:, i : i + channels], channels=channels)
...
def spatialAttention(X, Key, K, d, bn, bn_decay, is_training, top_k=32):
    ...
    attention = tf.where(
        tf.math.greater_equal(attention, min_),
        attention,
        tf.ones_like(attention) * (-(2**32) + 1),
    )
...
def STAttBlock(
    ...
    XS = STHolistic_tf(
        XS, is_encoder=is_encoder, pre_x=pre_x, channels=channels, input_len=input_len
    )
    print(XS[:, :, -N:].shape, XS.shape)
    HS = spatialAttention(
        XS[:, :, -N:], XS, K, d, bn, bn_decay, is_training, top_k=top_k
    )
    H = fusionGate(HS, HT)
    return tf.add(X, H)
...
def TS_TBLN(
    ...
    with tf.name_scope("BridgeTrans_Encoder_1"):
        X = BridgeTrans(
            X, X + STE_P, STE_Q + X_All[:, P:], K, d, bn, bn_decay, is_training
        )
    ...
    for i in range(L):
        with tf.name_scope(f"decoder_num_blocks_{i}"):
            X = STAttBlock(
                ...
                pre_x=tf.concat(
                    [
                        XS[:, -channels:] + X_All[:, P - channels : P],
                        STE_P[:, -channels:],
                    ],
                    axis=-1,
                ),
    ...
    with tf.name_scope("BridgeTrans_Decoder_1"):
        X = BridgeTrans(
            X, X + STE_Q, STE_P + X_All[:, :P], K, d, bn, bn_decay, is_training
        )

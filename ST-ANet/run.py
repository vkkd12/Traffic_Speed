...
class Model(tf.Module):
    def __init__(self, para):
        ...
        self.feature_dense = tf.keras.layers.Dense(
            self.para.emb_size, name="feature_embed"
        )
        self.projection_dense = tf.keras.layers.Dense(
            self.para.emb_size, name="projection_dense"
        )
        ...
        self.decoder_lstm_dep = decoder.lstm(
            self.para.batch_size * self.para.site_num,
            self.para.output_length,
            self.para.hidden_layer,
            self.para.hidden_size,
        ...
        self.day_emb_layer = tf.keras.layers.Embedding(
            32, self.para.emb_size, embeddings_initializer=init, name="day_embed"
        )
        self.hour_emb_layer = tf.keras.layers.Embedding(
            24, self.para.emb_size, embeddings_initializer=init, name="hour_embed"
        )
        ...
        _dummy_features = tf.zeros(
            [
                self.para.batch_size * self.para.input_length,
                self.para.site_num,
                self.para.features,
            ],
            dtype=tf.float32,
    ...
    def forward(self, features, day, hour):
        ...
        x = self.encoder_transformer.encoder(
            speed=features_emb,
            day=in_day,
            hour=in_hour,
    ...
    def accuracy(self, label, predict):
        ...
        cor = np.mean(
            np.multiply((label - np.mean(label)), (predict - np.mean(predict)))
        ) / (np.std(predict) * np.std(label))
    ...
    def train_step(self, features, day, hour, label):
        ...
        # Collect all trainable variables using TF 2.x API
        # tf.Module automatically tracks variables from constituent layers
    ...
    def run_epoch(self):
        ...
        train_dataset = iterate.next_batch(
            batch_size=self.para.batch_size, epochs=self.para.epochs, is_training=True
        )
        ...
        for batch in train_dataset:
            ...
            print(
                "after %d steps, the training average loss value is : %.6f"
                % (step, loss.numpy())
            )
            ...
            if step % 10 == 0:
                ...
                if max_rmse > rmse_error:
                    print(
                        "the validate average rmse loss value is : %.6f" % (rmse_error)
                    )
                    max_rmse = rmse_error
                    checkpoint = tf.train.Checkpoint(model=self)
                    checkpoint.save(
                        file_prefix=os.path.join(self.para.save_path, "model")
                    )
    ...
    def evaluate(self):
        ...
        test_dataset = iterate_test.next_batch(
            batch_size=self.para.batch_size, epochs=1, is_training=False
        )
        ...
        label_list = np.reshape(
            np.array(label_list, dtype=np.float32),
            [-1, self.para.site_num, self.para.output_length],
        ).transpose([1, 0, 2])
        predict_list = np.reshape(
            np.array(predict_list, dtype=np.float32),
            [-1, self.para.site_num, self.para.output_length],
        ).transpose([1, 0, 2])
        if self.para.normalize:
            label_list = np.array(
                [
                    self.re_current(np.reshape(site_label, [-1]), max_val, min_val)
                    for site_label in label_list
                ]
            )
            predict_list = np.array(
                [
                    self.re_current(np.reshape(site_label, [-1]), max_val, min_val)
                    for site_label in predict_list
                ]
            )
        else:
            label_list = np.array(
                [np.reshape(site_label, [-1]) for site_label in label_list]
            )
            predict_list = np.array(
                [np.reshape(site_label, [-1]) for site_label in predict_list]
            )

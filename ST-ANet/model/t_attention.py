...
def T_attention(hiddens, hidden, hidden_units=64, num_heads=1, num_blocks=4, dropout_rate=0.0):
    ...
    :return:
    '''
    with tf.name_scope("temporal_attention"):
        enc = tf.keras.layers.Dense(hidden_units)(hiddens)
        dec = tf.keras.layers.Dense(hidden_units)(hidden)
    ...
    print('s_attention dec shape is : ', dec.shape)
    return dec
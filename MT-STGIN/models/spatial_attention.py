...
def multihead_attention(queries,
    ...
    with tf.name_scope(scope):
        ...
        if dense_layers is not None:
            ...
            Q = tf.keras.layers.Dense(num_units, activation=tf.nn.relu)(queries)  # (N, T_q, C)
            K = tf.keras.layers.Dense(num_units, activation=tf.nn.relu)(keys)  # (N, T_k, C)
            V = tf.keras.layers.Dense(num_units, activation=tf.nn.relu)(keys)  # (N, T_k, C)
...
class SpatialTransformer(tf.Module):
    def __init__(self, arg):
        super().__init__(name='spatial_transformer')
        self.arg = arg
        ...
        # Pre-create layers for each block to ensure proper weight management
        self.attn_layers = []
        self.ff_layers = []
